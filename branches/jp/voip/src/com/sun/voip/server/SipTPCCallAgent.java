...
//                  + "the bridge does not support!");
                ...
                Logger.println("Authentication challenge placing call");
                ClientTransaction origTrans = responseReceivedEvent.getClientTransaction();
            
                ClientTransaction retryTrans =
                    SipServer.getSipSecurityManager().handleChallenge(response, 
                        origTrans.getBranchId(), origTrans.getRequest());
                retryTrans.sendRequest();
                
                // keep our existing listener to respond to the retried
                // transaction
                this.clientTransaction = retryTrans;
                return;
     ...
     */
    private void handleCallParticipantInvited(Response response, 
	    ClientTransaction clientTransaction) 
	...
	if (Logger.logLevel >= Logger.LOG_SIP) {
            Logger.println("handleCallParticipantInvited " + cp
		+ " status " + statusCode + " " + response.getReasonPhrase()); 
	}

	if (Logger.logLevel >= Logger.LOG_SIP) {
            Logger.println("handleCallParticipantInvited , displayname " 
	        + displayName);
	}
	...
	CallIdHeader callIdHeader = (CallIdHeader)
		...
		Logger.error("Call " + cp + " Invalid SDP in OK Response! "
		    + e.getMessage());
		...
		if (cp.getJoinConfirmationTimeout() == 0) {
            ...
            Logger.writeFile("Call " + cp 
		+ " Ignoring response: " + response.getReasonPhrase());
