...
public class SipIncomingCallAgent extends CallSetupAgent implements SipListener {
	...
	Logger.println(
	    "SipIncomingCallAgent:  Got an INVITE from " + from + " to " + to);

	Logger.writeFile(request.toString());

        if (Logger.logLevel >= Logger.LOG_SIP) {
	    Logger.println(
		"SipIncomingCallAgent:  Adding listener for call id " + sipCallId);
	}
	    ...
	    if (st == null) {
		st = 
		    SipServer.getSipProvider().getNewServerTransaction(request);
	    } 

            sipServerCallback.addSipListener(sipCallId, this);

    	    InetSocketAddress isa = new InetSocketAddress(Bridge.getPublicHost(), 
                                  callHandler.getReceiveAddress().getPort());
	    if (isa == null) {
		Logger.println(
		    "SipIncomingCallAgent:  can't get receiver socket!");
	        terminateCall();
		return;
	    }
